...
int MessageTreeItem::childCount() const
{
  ...
  if ( introspection_->value->is_array_ && type_ != ArrayElement ) {
    ...
    if ( item_data.typeId() == QMetaType::QVariantList ) {
      auto *arr_msg = static_cast<const QVariantList *>( item_data.data() );
      return static_cast<int>( arr_msg->size() );
    }
    if ( item_data.typeId() == qMetaTypeId<qml6_ros2_plugin::Array>() ) {
      const auto *array = static_cast<const qml6_ros2_plugin::Array *>( item_data.data() );
      return array->length();
    }
    return 0;
  }
...
QVariant *MessageTreeItem::getItemDataPtr() const
{
  ...
  if ( type_ == ArrayElement ) {
    if ( item_data->typeId() == qMetaTypeId<qml6_ros2_plugin::Array>() ) {
      auto *array = static_cast<qml6_ros2_plugin::Array *>( item_data->data() );
  ...
  if ( item_data->typeId() != QMetaType::QVariantMap ) {
    QML_ROS2_PLUGIN_ERROR( "MessageTreeItem::getItemDataPtr: Unexpected type '%d'!",
                           item_data->typeId() );
    return nullptr;
  }
...
QVariant MessageTreeItem::getItemData() const
{
  ...
  if ( item_data->typeId() == qMetaTypeId<qml6_ros2_plugin::Time>() ) {
    auto time = static_cast<qml6_ros2_plugin::Time *>( item_data->data() );
    builtin_interfaces::msg::Time t = time->getTime();
    return name_ == "sec" ? t.sec : t.nanosec;
  }
  if ( item_data->typeId() == qMetaTypeId<qml6_ros2_plugin::Duration>() ) {
    auto duration = static_cast<qml6_ros2_plugin::Duration *>( item_data->data() );
...
{
  if ( QMetaType type_id = getItemData().metaType();
       value.metaType() != type_id && !value.convert( type_id ) )
    return false;
  if ( !parent_item_ ) {
    *root_data_ = value;
    return true;
  ...
  if ( item_data->typeId() == qMetaTypeId<qml6_ros2_plugin::Duration>() ) {
    ...
    if ( !ok )
      return false;
    *duration = qml6_ros2_plugin::Duration( d );
    return ok;
  }
  if ( item_data->typeId() != QMetaType::QVariantMap ) {
...
QModelIndex MessageItemModel::parent( const QModelIndex &child ) const
{
  ...
  return parentItem != root_item_.get() ? createIndex( parentItem->row(), 0, parentItem )
                                        : QModelIndex{};
}
